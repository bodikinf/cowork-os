...
<img src="docs/banner.png" alt="cowork-os — the open-source operating system for Claude Cowork" width="100%" />

# cowork-os

**An open-source operating system for Claude Cowork.**
*Un sistema operativo open-source per Claude Cowork.*
...
**🟢 Path A · Guided setup (recommended).** Copy the folder into a new Cowork project, paste the contents of [`INSTALL.md`](./INSTALL.md) into the chat, and answer ~6 questions. Claude interviews you and **generates your whole workspace pre-configured** — folders, copy, and the recurring automations that fit you. You don't need to know what any of the files are; that's Claude's job. It even surfaces capabilities most people don't know exist (scheduled automations, mission mode, skills). Already have notes, a deck, or a messy folder? Drop it in `_inbox/` and Claude organizes it into the workspace for you.

**Path B · Manual.** Prefer to do it by hand, or not on Cowork? Fill [`PROJECT_INSTRUCTIONS.md`](./PROJECT_INSTRUCTIONS.md) and the `context/` templates yourself — see [`GETTING_STARTED.md`](./GETTING_STARTED.md). Every file ships as a skeleton **plus a sanitized Yempik example**.
...
### Avvio rapido — due modi

**🟢 Via A · Setup guidato (consigliato).** Copia la cartella in un progetto Cowork nuovo, incolla il contenuto di [`INSTALL.md`](./INSTALL.md) nella chat e rispondi a ~6 domande. Claude ti intervista e **genera tutto il workspace già configurato** — cartelle, copy e le automazioni ricorrenti adatte a te. Non devi sapere cosa siano i file: è compito di Claude. Ti propone anche funzioni che quasi nessuno sa che esistono (automazioni schedulate, mission mode, skill). Hai già note, un deck o una cartella disordinata? Buttala in `_inbox/` e Claude la organizza nel workspace per te.

**Via B · Manuale.** Preferisci farlo a mano, o non sei su Cowork? Compila tu [`PROJECT_INSTRUCTIONS.md`](./PROJECT_INSTRUCTIONS.md) e i template in `context/` — vedi [`GETTING_STARTED.md`](./GETTING_STARTED.md). Ogni file è uno scheletro **più un esempio Yempik sanitizzato**.
...
Created by **Yempik** — a software house specialized in software, automation and AI agents — and maintained by Raffaele. If this helped you, a star ⭐ and a tag on LinkedIn make our day.
